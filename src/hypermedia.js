 ...
 */
 class HyperMedia extends Client{
	...
	constructor(config){
		super(config) 
		var x = this
		Konekti.daemon( function(){ return (Konekti.client[x.media] !== undefined && Konekti.client[x.media] !== null)},
			function(){ Konekti.client[x.media].addListener(x.id) })
	}
 ...
 */
Konekti.hypermedia = function(id, layout, media, scripts, config={}, callback=function(){}){
	Konekti.add({'plugin':'hypermedia', 'setup':['body', id, layout, media, scripts, config]}, callback)
}