...
class Ace extends Editor{
		...
		Konekti.daemon(
			...
			function (){ 
				...
				if( x.code !== '' ){
					x.code.cid = x.id
					x.code.mode = x.mode
					Konekti.plugin['ace'].register(x.code, x.edit)
				...
				x.vc().getElementsByTagName('textarea')[0].addEventListener('keyup', function(event){ 
					for( var i=0; i<x.listener.length; i++ ){
						var l = Konekti.client[x.listener[i]]
	...
	scrollTop(pos){
		var x=this
		var sui = x.vc().getElementsByClassName('ace_scroller')[0]
		var sbui = x.vc().getElementsByClassName('ace_scrollbar-v')[0].getElementsByClassName('ace_scrollbar-inner')[0]
		var tout
		function check(){
			var h = parseInt(sbui.style.height, 10)
			var ls = parseInt(sui.style.lineHeight,10)
			var tlines = h/ls
	...
	resize( width, height ){
		...
		if(this.edit!==undefined) this.edit.env.onResize(true)
	}
}
 ...
 */
Konekti.ace = function(id, initial, mode='', theme='', code='', config={}, callback=function(){}){ 
	Konekti.add({'plugin':'ace', 'setup':['body',id,initial,mode,theme,code,config]},callback)
}