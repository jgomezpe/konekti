...
class Canvas extends Editor{
	 ...
	 */
	setText(txt){
		if(typeof txt === 'string')	this.commands = JSON.parse(txt)
		else this.commands = txt
		this.redraw()
	}
}
 ...
 */
Konekti.canvas = function(id, initial, custom_commands={}, config={}, callback=function(){}){
	Konekti.add({'plugin':'canvas', 'setup':['body', id, initial, custom_commands, config]}, callback)
}
